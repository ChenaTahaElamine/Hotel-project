...
const AddImageArticleBlog = () => {
    ...
    const articleBlog = objArticleBlog.getArticleBlogImage();
    ...
    const handleFetchImages = async () => {
        if (articleBlog.media.length > 0) {
            const imageUrlsToFetch = [];

            for (let index = 0; index < articleBlog.media.length; index++) { imageUrlsToFetch.push(articleBlog.media[index].chemin_fichier); }
            ...
            for (const imageUrl of imageUrlsToFetch) {
                try {
                    const response = await axios.get(`${ReturnLeNomDeDominDeApi}/api/admin/image-or-vidio/showFileName/${imageUrl}`, {
                        responseType: 'blob',
                        headers: {
                            'Authorization': `Bearer ${objArticleBlog.getToken()}`,
                        },
                    });
    ...
    const onSubmit = async (data) => {
        if (data.image[0]) {
            ...
            try {
                const response = await axios.post(`${ReturnLeNomDeDominDeApi}/api/admin/image-or-vidio/store-img-article-blog/${articleBlog.id}`, formData, {
                    headers: {
                ...
                navigate("/images_or_vidios/ArticleBlogMedia");
    ...
    const onDelete = async (fileName) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                await objArticleBlog.deleteImage(fileName);
                // const res = await AddImageChambreFileConnection.delete(objChambreTable.getToken(), fileName);
                navigate("/images_or_vidios/ArticleBlogMedia");
                // return res;
    ...
    return (
        <div id="main-wrapper" data-layout="vertical" data-navbarbg="skin6" data-sidebartype="full"
            ...
            <div className="page-wrapper">
                ...
                <PageBreadcrumb location={"Les images des article blog"} buttonReteur={"/images_or_vidios/ArticleBlogMedia"} />
                <div className='container-fluid'>

                    <div className="card">
                        <div className="card-body">
                            <h3 className="card-title">Le titre de l'article blog</h3>
                            ...
                            <div className="row">
                                ...
                                <div className="col-lg-7 col-md-7 col-sm-6">
                                    <h4 className="box-title mt-5">contenu</h4>
                                    <p>
                                        {articleBlog.contenu}
                                    </p>
                                    <h2 className="mt-5">
                                ...
                                <div className="col-lg-7 col-md-7 col-sm-6">
                                    <h4 className="box-title mt-5">date publication</h4>
                                    <p>
                                        {articleBlog.date_publication}
                                    </p>
                                    <h2 className="mt-5">
                                        {/* $153<small className="text-success">(36%off)</small> */}
                                    </h2>
                                ...
                                {articleBlog.commentateurs.length === 0 ? (
